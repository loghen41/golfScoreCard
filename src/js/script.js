...
function addPlayers() {
    ...
    for (p = 1; p <= playersValue; p++) {
        $(scoreCard).append("<div id = 'player" + p + "' class = 'col-sm-12 col-md-12 col-lg-12 col-xs-12 player defaultColor" + p + "'></div>");
    }
    addTrays();
...
function addHoles() {
    ...
    if (holesValue === "9") {
        for (h = 1; h <= holesValue; h++) {
            $(trayNum).append("<input type='number'  onchange='scoreMessage()' oninput='finalScores();' style='text-align: center;'  min = '1' class = 'col-sm-1 col-md-1 col-lg-1 col-xs-1 hole" + h + "'> </input>");
        }
    }
    else if (holesValue === "18") {
        for (h = 1; h <= holesValue / 2; h++) {
            if (trayNum === ".tray2") {
                $(trayNum).append("<input  type='number' onchange='scoreMessage();' oninput='finalScores();' style='text-align: center;' min = '1'  class = 'col-sm-1 col-md-1 col-lg-1 col-xs-1 hole" + (h + 9) + "' > </input>");
            } else {
                $(trayNum).append("<input  type='number' onchange='scoreMessage();' oninput='finalScores();' style='text-align: center;' min = '1'  class = 'col-sm-1 col-md-1 col-lg-1 col-xs-1 hole" + h + "' > </input>");
            }
        }
        ...
        if (holesValue === "9") {
            ...
            if (document.getElementsByClassName('hole1')[q].value !== "" && document.getElementsByClassName('hole2')[q].value !== "" && document.getElementsByClassName('hole3')[q].value !== "" && document.getElementsByClassName('hole4')[q].value !== "" && document.getElementsByClassName('hole5')[q].value !== "" && document.getElementsByClassName('hole6')[q].value !== "" && document.getElementsByClassName('hole7')[q].value !== "" && document.getElementsByClassName('hole8')[q].value !== "" && document.getElementsByClassName('hole9')[q].value !== "") {
                ...
                if (playerScore < totalPar) {
                    alert("You did so good, you should be in the Pros!");
                }
                else if (playerScore < (+totalPar + 9)) {
                    alert("You did really well!");
                }
                else if (playerScore > (+totalPar + 9)) {
                    alert("Well.... looking at your Score, you maybe should take up Mini-Golf.");
                }
...
function validateForm() {
    ...
    for (r = 1; r <= playersValue; r++) {
        nameArray.push(mainForm['name' + r]);
    }
